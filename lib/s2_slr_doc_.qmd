...
Initially 6 queries have been run against the 6 databases resulting in 8X_ hits as shown in @tbl-query-results in the [Appendix A](#appendix-a).
...
<!-- note this is the result, not the protocol -->
...
Records have been included if any of the following inclusion criteria (IC) holds:

- IC1 - the title, abstract and full text are in english
- IC2 - the publication is dated between 2000 and 2024

**exclusion criteria**

Records have been excluded if any of the following exclusion criteria (EC) holds:

- EC1 - the reference is to a book(chapter)
- EC2 - the submission has been retracted
...
As metrics like article citation count, author h-index and journal impact factor are not available from each of the databases, such metrics have not been considered in the selection process. 

Check against previously constructed list of relevant journals (repeat here?)

## Data extraction strategy

On the basis of table 3 drop two data source (with least hits), one of them Zenodo. The second reason for dropping Zenodo is first that as a "dataset repository" it is a different animal than the other data sources which are dedicated literature databases. 

Queries have been formulated for each data source. The output of each query has either been a bib file or a ris file. All bib files have been converted to ris files. The mapping of bibliographic elements to the ris fields differs per data sources (see @tbl-ris-fields). A script has been written to get to a uniform ris-format [Appendix A](#appendix-a). 

Describe how 6 data sources have been reduced to 4.

no ris but bib ...

The open access databases do not contain the full text articles if the articles are not open access. YY also doesn't contain abstracts. The absence of full text articles or abstracts is explained by constraints imposed by copyrights (source).

The initial list (@tbl-database-searches) contains a total of 84 records. An additional 6 records were found from other sources (@tbl-other-sources). The total is 90 records. This list includes preprints and conference papers. Filtering the journal articles (peer reviewed?) results in a list of 25 records (@tbl-journal-articles).
...
- A poster that helps explain fellow DBA students the this literature review process.
...
## Limitations
<!--
projects, since 2014 (@tbl-resultset). 
Synonyms
-->
...
In follow-on research it maybe interesting to include openAIRE (Open Access Infrastructure for Research in Europe) as a data source. Several articles in this review, such as @schummBusinessProcessCompliance2010, p. 11; and YYY p , reference research that has been performed under the 2008-2011 [COMPAS](https://cordis.europa.eu/project/id/215175) research project funded by the EU and coordinated by the Technische Universitaet Wien. More information about the research output of these projects will be available via OpenAIRE. When OpenAIRE will be included as a data source we will also again include Zenodo. Zenodo was created as part of the OpenAIRE initiative.

For reproducibility a future version of this literature reviews shall have the protocol altered. For commercial databases one and the same query can yield different results if is run in two different licensed versions of the same database. This has been found to be the case with Web of Science. It is assumed the user does not have precise information of the licenses that Academic institutions hold. As such WoS better not used in the early stages of the search. The protocol could instead dictate the use of open databases, such as OpenAlex, for the early stages of the search. Commercial databases would then be used in a later stage to get abstracts and full texts of selected articles that are not available from open databases.
