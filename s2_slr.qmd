...
Initially 6 queries have been run against the 6 databases resuling in __ hits as shown in @tbl-query-results in the appendix.
...
In the case of Web of Science (WoS) the four queries have been taken together using the "OR" operator. This final query yields 42 results when using the the Vlerick WoS subscription[^1].

| Search Date   | Search ID | Query URL | Query Results  | Hits |
|---------------|-----------|-----------|----------------|------|
| 25-02-2024    |           |           | [DOI][10795823]|  42  |
: Hits
...
Queries have been formulated for each datasource. The output of each query has either been a bib file or a ris file. All bib files have been converted to ris files. The mapping of bibliographic elements to the ris fields differs per datasources (see @tbl-ris-fields). A script has been written to get to a uniform ris-format, using the WoS ris-format as a template.

Describe how 6 datasources have been reduced to 4.

The diagram @diagram in the appendix shows the workings of the script.

no ris but bib ...


The open access databases do not contain the full text articles (check per database). YY also doesn't contain abstracts. The absense of full text articles or abstracts is explained by contraints imposed by copyrights (source).
...
The total number of records of the 4 resulting ris files is xx. After combining the 6 files and deduplicating records the total number of record is 49 (see appendix: @tbl-resultset).

Additionally 3 records have been added that were identified through other sources (see appendix: @tbl-additional-records)
...
Subtracting retracted articles, pre-prints, non-english articles and book(chapters) leaves a number of xx records.
...
```{python}
#| label: tbl-additional-records
#| tbl-cap: Additional records from other sources
#| 
from IPython.display import Markdown
from tabulate import tabulate
from lib.my_excel_lib import get_tables
...
Markdown(tabulate(
  xls2md(tables['AdditionalRecords']),
  headers=['Nr','Type','Title'],
  missingval=[''],
  maxcolwidths=[3, 10, 100],
  showindex=False,
