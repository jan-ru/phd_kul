...
A definition of the three central concepts is included in the appendix.
...
The initial searches were done all database fields. The search was subsequently made more specific by searching titles and abstracts and later titles, abstracts and keywords.
...
The total number of records of the 5 resulting ris files is xx. After combining the 5 files and deduplicating records the total number of record is 49 (see document: List 1 - Selected Titles). Additionally 3 records have been added that were identified through other sources.

Of the total of 52 journal articles and conference papers the titles and abstracts have been read and a total of yy articles have been excluded based on: 
...
The process to get to the final zz papers included in this literature study is visualized in the flowchart in @fig-slr-flowchart in the appendix.
...
Queries have been formulated for each datasource. The output of each query has either been a bib file or a ris file. All bib files have been converted to ris files. The mapping of bibliographic elements to the ris fields differs per datasources (see @tbl-ris-fields). A script has been written to get to a uniform ris-format. The diagram @diagram in the appendix shows the workings of the script.
...
An upcoming version of the review will be part of the PhD work. The aim is to get a few by-products out of this review:
...
SSRN show '-' in all fields as SSRN

\newpage
**Merge bibliographic data**

![ris2xls.py sript structure](static/s2_slr_script_archimate.jpg)
...
[^2]: Wikipedia page [RIS format](https://en.wikipedia.org/wiki/RIS_(file_format))
